...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Recipe generation request from user: ${userId}`);

    const { dietary, cuisine, prepTime, calories, allergies, messages, userMessage } = await req.json();
    ...
    // If there are existing messages (conversation), handle chat mode
    if (messages && messages.length > 0 && userMessage) {
      console.log("Continuing recipe conversation");
      ...
      const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are a professional nutritionist and chef helping users refine their recipes. When the user asks for changes, modify the recipe accordingly and provide the complete updated recipe with all details (name, description, prep/cook time, servings, ingredients, instructions, and nutrition)."
            },
            ...messages,
            {
              role: "user",
    ...
    console.log("Generating initial recipe with preferences:", { dietary, cuisine, prepTime, calories, allergies });
