...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`generate-pct-prompts request from user: ${userId}`);

    const { pillar, topic, type, responses, fresh } = await req.json();

    // Input validation
    if (typeof pillar !== 'string' || pillar.length > 100) {
      return new Response(JSON.stringify({ error: 'Invalid pillar parameter' }), {
    ...
    if (type && !['summary', 'prompts'].includes(type)) {
      return new Response(JSON.stringify({ error: 'Invalid type parameter' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    if (type === 'summary' && (!Array.isArray(responses) || responses.length > 20)) {
      return new Response(JSON.stringify({ error: 'Invalid responses parameter' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
